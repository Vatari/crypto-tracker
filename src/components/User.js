const User = ({ prices, mainPrices, img }, props) => {
  ...
  const qty = Object.values(prices).map((p) => p.q);

  //console.log(price);
  const percent = Number((mainPrices.o - mainPrices.c) / mainPrices.o);
  //const lastQty = Number(props.q).toFixed(3);
  const formatDate = () => {
    const dateFormatted = new Date().toLocaleString();
    return dateFormatted;
  };

  const currencyImg = (pair) => {
    if (pair === "ETHUSDT") {
      return require("../assets/images/eth-logo.png");
    }
    if (pair === "BTCUSDT") {
      return require("../assets/images/btc_logo.png");
    }
  };
  ...
  return (
    <tr>
      ...
      <td
        style={{
          background:
            !mainPrices.c || mainPrices.c === price
              ? "white"
              : price > mainPrices.c
              ? "green"
              : "red",
      ...
      >
        {Number(price).toFixed(2)} USDT
      </td>
      <td>{Number(qty).toFixed(3)}</td>
